...
def test(X, Y, perms=10000, method='pearson', tail='two-tail'):
    ...
    if spatial.distance.is_valid_dm(X, tol=0.001) == False and spatial.distance.is_valid_y(X, tol=0.001) == False:
        raise ValueError('X is not a valid condensed or redundant distance matrix')
    if spatial.distance.is_valid_dm(Y, tol=0.001) == False and spatial.distance.is_valid_y(Y, tol=0.001) == False:
    ...
    if perms >= n or perms == 0:
        ...
        for i, order in enumerate(permutations(range(m))):
            ...
            spatial.distance._distance_wrap.to_vector_from_squareform_wrap(Y_residuals_as_matrix_permuted,
                                                                           Y_residuals_permuted)
