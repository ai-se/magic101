...
class DE(object):
  ...
  def Tune(self):
    def isBetter(new, old):
      return new < old if self.goal == "PF" or self.goal == "MRE" else new > old
...
class DE_Tune_ML(DE):
  def __init__(self, learner, params_distribution, goal, target_class,
               num_population=10, repeats=60, f=0.75, cr=0.3, life=5):
    self.learner = learner
  ...
  def best(self):
    if self.goal == "PF" or self.goal == "MRE":  # the less, the better.
