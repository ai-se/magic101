...
class MOEAD(object):
    def __init__(self, population, toolbox, mu, cxpb, mutpb, ngen=0, maxEvaluations=0,
        ...
        if population:
            ...
            fitnesses = self.toolbox.map(self.toolbox.evaluate4, self.population)
        ...
        # Operators
        ## By convention, toolbox.mutate3 and toobox.mate3 should handle crossovers and mutations.
        try:
            self.toobox.mate3
            self.toolbox.mutate3
        except Exception as e:
            print
            "Error in MOEAD.__init__: toolbox.mutate3 or toobox.mate3 is not assigned."
            raise e
    ...
    def execute(self):
        ...
        self.evaluations_ = 0
        print
        "POPSIZE:", self.populationSize_

        self.indArray_ = [self.toolbox.Individual4() for _ in range(self.n_objectives)]
        ...
        while count < LIFE and self.evaluations_ < self.maxEvaluations:
            ...
            for i in range(self.populationSize_):
                ...
                children = self.toobox.mate3(parents[0], parents[1])

                # Apply mutation
                children = [self.toolbox.mutate3(child) for child in children]
                # Evaluation
                offspring = []

                for child in children:
                    fit = self.toolbox.evaluate4(child[0])
                    self.evaluations_ += 1
            ...
            fits_new = [ind.fitness.values[0] for ind in self.population]

            if median(fits_new) >= median(fits_old):
                count += 1
            fits_old = fits_new

        return self.population, g
    ...
    def initUniformWeight(self):
        ...
        if self.n_objectives == 2:
                ...
                for j in range(m):
                    if i + j <= m:
                        ...
                        try:
                            ...
                            print
                            "Error creating weight with 3 objectives at:"
                            print
                            "count", count
                            print
                            "i", i
            ...
            try:
                with open(file_, 'r') as f:
                    ...
                    for aux_ in f:
                        ...
                        try:
                            ...
                            "value:", value
                            print
                            "lambda_", len(self.lambda_), "*", len(self.lambda_[0])
                            print
                            i, j, aux_
                            print
                            e
                ...
                print
                "initUniformWeight: failed when reading for file:", file_
                print
                e
                raise e
    ...
    def initIdealPoint(self):
        for i in range(self.n_objectives):
            self.indArray_[i] = self.toolbox.Individual4()
            self.indArray_[i].fitness.values = self.toolbox.evaluate4(self.indArray_[i])
    ...
    def fitnessFunction(self, individual, lambda_):
        ...
        if self.functionType_ == "_TCHE1":
            ...
            print
            "MOEAD.fitnessFunction: unknown type", self.functionType_
            raise NotImplementedError
